...
CYCLE_PREFIX_PATTERN = re.compile(
    ...
    r"r_button=(?P<r_button>[01xXzZ])\s+"
    r"g_button=(?P<g_button>[01xXzZ])\s+"
    r"key=(?P<key>[01xXzZ])\s+"
    r"start=(?P<start>[01xXzZ])\s+"
    r"test=(?P<test>[01xXzZ])\s+"
    r"rts=(?P<rts>[01xXzZ])\s+"
    r"rtr=(?P<rtr>[01xXzZ])\s+"
    r"v_in=(?P<v_in>[01xXzZ])\s+"
        ...
        cycle_rows.append(
            {
                ...
                "r_button": match.group("r_button"),
                "g_button": match.group("g_button"),
                "key": match.group("key"),
                "start": match.group("start"),
                "test": match.group("test"),
                "rts": match.group("rts"),
                "rtr": match.group("rtr"),
                "v_in": match.group("v_in"),
...
def main() -> int:
    ...
    for log_path in log_files:
        ...
        if not cycle_rows:
            ...
            print(
                f"WARNING: No B05 cycle records "
                f"found in {log_path}"
            )
