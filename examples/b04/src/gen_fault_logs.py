...
CYCLE_PREFIX_PATTERN = re.compile(
    ...
    r"RESTART=(?P<restart>[01xXzZ])\s+"
    r"AVERAGE=(?P<average>[01xXzZ])\s+"
    r"ENABLE=(?P<enable>[01xXzZ])\s+"
    r"DATA_IN=(?P<data_in>[01xXzZ])\s+"
        ...
        cycle_rows.append(
            {
                ...
                "request1": match.group("restart"),
                "request2": match.group("average"),
                "request3": match.group("enable"),
                "request4": match.group("data_in"),
...
def main() -> int:
    ...
    for log_path in log_files:
        ...
        if not cycle_rows:
            ...
            print(
                f"WARNING: No B04 cycle records "
                f"found in {log_path}"
            )
