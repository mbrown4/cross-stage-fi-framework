...
DFF_RE = re.compile(
    r'(?P<indent>^[ \t]*)'
    r'(?P<cell>sky130_fd_sc_hd__(?:dfrtp|dfstp)_\d+)\s+'
...
def build_replacement(indent, cell, inst, ports, idx):
    p = parse_ports(ports)
    ...
    if "dfrtp" in cell:
        ctrl = fix_signal(p.get("RESET_B"))
        if not all([clk, d, q, ctrl]):
            raise RuntimeError(f"Instance {inst} missing CLK/D/Q/RESET_B")
        wrapper = "FI_DFF_DFRTP_FAULTY"
        extra_port = "RESET_B"

    elif "dfstp" in cell:
        ctrl = fix_signal(p.get("SET_B"))
        if not all([clk, d, q, ctrl]):
            raise RuntimeError(f"Instance {inst} missing CLK/D/Q/SET_B")
        wrapper = "FI_DFF_DFSTP_FAULTY"
        extra_port = "SET_B"

    else:
        raise RuntimeError(f"Unsupported cell type for {inst}: {cell}")

    replacement = (
        f"{indent}{wrapper} {inst} (\n"
        f"{indent}  .CLK({clk}),\n"
        f"{indent}  .D({d}),\n"
        f"{indent}  .{extra_port}({ctrl}),\n"
...
def rewrite_netlist(netlist_text, mapping_entries):
    ...
    for idx, (rtl_name, bit, inst) in enumerate(mapping_entries):
        ...
        inst_re = re.compile(
            rf'(?P<indent>^[ \t]*)'
            rf'(?P<cell>sky130_fd_sc_hd__(?:dfrtp|dfstp)_\d+)\s+'
        ...
        replacement = build_replacement(indent, cell, inst, ports, idx)
